...
// Provider: 'gemini' (free tier, default) · 'ollama' (free, local) · 'anthropic' (paid, best).
const PROVIDER = process.env.MUSE_PROVIDER || 'gemini'
...
const CALLERS = { gemini: callGemini, ollama: callOllama, anthropic: callAnthropic }
...
app.listen(PORT, () => {
  ...
  console.log(
    `  provider: ${PROVIDER}  ·  model: ${activeModel()}` +
      (fallbackActive ? `  ·  fallback: ${FALLBACK}` : ''),
  )
  if (PROVIDER === 'gemini' && !process.env.GEMINI_API_KEY)
    console.log('  ⚠  GEMINI_API_KEY not set — get a free key at https://aistudio.google.com/apikey')
  if ((PROVIDER === 'ollama' || FALLBACK === 'ollama') && OLLAMA_MODEL)
    console.log(`  (Ollama path uses "${OLLAMA_MODEL}" — pull it with: ollama pull ${OLLAMA_MODEL})`)
})
